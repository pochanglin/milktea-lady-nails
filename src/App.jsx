...
const App = () => {
    ...
    const copyText = () => {
        ...
        return `𝙼𝙸𝙻𝙺𝚃𝙴𝙰.𝙻𝙰𝙳𝚈_ 𝙽𝙰𝙸𝙻 預約明細\n` +
            ...
            `⏰ 時段：${booking.time || '未填'}\n` +
            `━━━━━━━━━━━━━\n` +
            (selectedStyleObj ? `🍂 款式：${selectedStyleObj.name} $${selectedStyleObj.price}\n` : "") +
            (selectedRemovalObj ? `🧼 卸甲：${selectedRemovalObj.name} $${selectedRemovalObj.price}\n` : "") +
    ...
    const copyToClipboard = async () => {
        ...
        if (navigator.share) {
            try {
                ...
                // 使用者取消分享或發生錯誤時，不進行任何操作或降級處理
                console.log('取消分享或不支援', err);
            }
        } else {
            // 降級處理：不支援分享 API 時，使用原本的複製功能
            ...
            <div className="w-full max-w-md bg-[#F2EDE7] rounded-[2.5rem] shadow-2xl overflow-hidden border border-white/40">
                ...
                {activeTab === 'rules' && (
                    <div className="p-8 space-y-8 animate-in fade-in duration-500">
                        ...
                        <button onClick={() => setActiveTab('price')} className="w-full py-4 bg-[#8E735B] text-white rounded-full font-bold tracking-[0.2em] text-[12px] shadow-lg">前往預約計價</button>
                    </div>
                )}
